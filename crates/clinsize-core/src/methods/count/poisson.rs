//! Two-sample Poisson sample size and power for event counts.
//!
//! Signorini (1991) Wald test for the log rate ratio. This is the limiting
//! case of the negative binomial method as the dispersion parameter k → 0
//! (Var(Y) = μ).
...
/// Inputs for two-sample Poisson comparison of event rates.
...
/// Results for two-sample Poisson comparison.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
...
pub fn variance_factor(
    ...
    (1.0 / mu1) + (1.0 / r) * (1.0 / mu2)
}

pub fn validate(input: &PoissonInput) -> Result<()> {
    ...
    match input.solve_mode {
        ...
        SolveMode::DetectableEffect => {
            return Err(Error::UnsupportedMethod(
                "detectable effect solve mode is not implemented for Poisson".into(),
            ));
        }
    }
...
pub fn calculate(input: PoissonInput) -> Result<PoissonResult> {
    validate(&input)?;

    let mut warnings = vec![
        CalculationWarning::new(
            "poisson_wald",
            "Uses a Wald test for the log rate ratio with Poisson variance Var(Y) = μ (no overdispersion).",
        ),
        CalculationWarning::new(
            "fixed_exposure",
            "Assumes fixed exposure time per subject and independent Poisson counts.",
        ),
    ];
...
pub fn achieved_power(n_control: u32, _n_treatment: u32, input: &PoissonInput) -> f64 {
...
fn closed_form_control_n(input: &PoissonInput, target_power: f64) -> u32 {
...
fn solve_control_n(input: &PoissonInput, target_power: f64) -> Result<u32> {
...
mod tests {
    ...
    fn rejects_equal_rates() {
        let input = sample_size_input(2.0, 2.0, 0.05, 0.8, 1.0);
        assert!(validate(&input).is_err());
    }

    // Reference values filled in after empirical verification against the engine;
    // see validation/count/poisson/reference-output.md for the Signorini (1991)
    // derivation. The formula gives n_control ≈ 25 for λ₁=2, λ₂=1, α=0.05,
    // power=0.8, ratio=1, exposure=1.

    #[test]
    fn matches_signorini_halving_rate() {
        // λ₁=2, λ₂=1, exposure=1, ratio=1, α=0.05, power=0.8
        let result = calculate(sample_size_input(2.0, 1.0, 0.05, 0.8, 1.0)).expect("calculate");

        assert_eq!(result.n_control, 25);
        assert_eq!(result.n_treatment, 25);
        assert_eq!(result.total_n, 50);
        assert_relative_eq!(result.rate_ratio, 0.5, epsilon = 1e-12);
        assert_relative_eq!(result.achieved_power, 0.8, epsilon = 0.02);
    }

    #[test]
    fn matches_signorini_doubling_rate() {
        // λ₁=5, λ₂=10, exposure=1, ratio=1, α=0.05, power=0.8
        let result = calculate(sample_size_input(5.0, 10.0, 0.05, 0.8, 1.0)).expect("calculate");

        assert_eq!(result.n_control, 5);
        assert_eq!(result.n_treatment, 5);
        assert_eq!(result.total_n, 10);
        assert_relative_eq!(result.rate_ratio, 2.0, epsilon = 1e-12);
    }

    #[test]
    fn power_mode_uses_supplied_control_n() {
        let input = PoissonInput {
