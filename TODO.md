...
Medium
  ...
  vs gsDesign's ~1e-6. Upgrade the MVN integration; as a short first step, surface the current tolerance in exports
  (currently documented only in validation/).
...
Recently completed
  - Collapse the Tauri-side dispatch duplication. Merged in PR #13: two generic Tauri method commands and typed
  frontend helpers now replace the per-method calculation/Markdown commands and literal invoke() call sites.
