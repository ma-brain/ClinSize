TODO — what's left to fix
  ...
  3. Collapse the dispatch duplication (~1,500 lines): the 19-method match is hand-written 4× in dispatch.rs,
  ...
  Medium
  4. Group sequential / Dunnett numerical precision: fixed 12,000-point Halton QMC gives ~0.02 boundary accuracy
  ...
  5. find_minimum_integer monotonicity guard — binary search over power assumes a monotone predicate; add a
  walk-down check after the search.
  6. Frontend tests: zero unit/component tests (svelte-check only). Start with vitest on buildInput()
  round-trips and summarizeResult.
  7. Project history shows "—" for one-sample, paired, multiplicity, and group-sequential records
  (summarizeResult only knows totalN).

  Low / cleanup
  8. odds_ratio/risk_ratio validate() don't reject control_n in sample-size mode or power in power mode (every
  other method does).
  9. iso_timestamp() returns raw Unix seconds despite the name (project files show "createdAt": "1752…");
  newId() can collide within one millisecond.
  10. InflationHeuristic one-impl trait in group_sequential.rs (violates the repo's own "no premature
  abstraction" rule); design methods list empty solve modes in clinsize list.
  Process
  11. Merge PR #3 once you've reviewed it — CI is green and branch protection now requires it.
  12. Recompute any previously exported MMRM results — the old outputs are undersized by ~`(1+(k−1)ρ)/(1−ρ)`;
